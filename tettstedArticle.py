...
class TettstedArticle:
    ...
    def write_infobox(self):
        ...
        text+= "| Fylke = " + self.fylke.get_number() + "\n" 
        text+= "| Kommune = " + self.muni.get_wiki_link() + "\n" 
        text+= "| lat_deg = " + self.lat +"\n" 
        text+= "| lon_deg = " + self.long + "\n" 
        text+= "| Einwohner = {{Metadaten Einwohnerzahl Ort NO|" + self.number + "}}\n" 
        text+= "| Stand = {{EWD|Ort NO|}}\n" 
        text+= "| Fläche = {{Metadaten Fläche Ort NO|" + self.number + "}}\n" 
        text+= "| Höhe = \n" 
        text+= "| Straßen = \n" 
        text+= "| Schienen = \n" 
        text+= "| Flughafen = \n" 
    ...
    def write_basic_structure(self):
        text = "== Geografie ==\n" + self.name
        text += " ist ein sogenannter [[Tettsted]], also eine Ansiedlung, die für statistische Zwecke als eine Ortschaft gewertet wird.<ref name=\"ssb\" />\n\n"
        text += reference.get_source_norgeskart(self.name)
        text += "\n\n== Geschichte =="
        text += "\n\n== Wirtschaft und Infrastruktur =="
        text += "\n\n== Name ==\n"
        text += self.name + " wurde im Jahr ... als ''...'' erwähnt. Der Name setzt sich aus den beiden Bestandteilen „...“ und „...“ zusammen, erster leitet sich von ... ab, „...“ steht hingegen für „...“." +  reference.get_source_stadnamn(self.name)
