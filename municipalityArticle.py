...
class MunicipalityArticle:
    ...
    def write_first_paragraph(self):
        text = "'''" + self.name + "'''" + " ist eine [[Kommune (Norwegen)|]] im [[Norwegen|norwegischen]] [[Fylke]] "\
               + self.fylke.get_wiki_link() + ". Die Kommune hat {{EWZ|NO|" + self.muni_nr +\
               "}} Einwohner (Stand: {{EWD|NO|" + self.muni_nr + "}}). Verwaltungssitz ist "
    ...
    def write_geography(self):
        ...
        water_area = str(muniArea.get_water_area(self.muni_nr)).replace(".", ",")

        text += " Die Gesamtfläche der Kommune beträgt {{FL|NO|" + self.muni_nr + \
                "|2}}&nbsp;km², wobei [[Binnengewässer]] zusammen " + water_area + "&nbsp;km² ausmachen."
    ...
    def write_new_fylke(self):
        ...
        if fylke_name == "Trøndelag":
            text += "31. Dezember 2017 gehörte " + self.name + \
                    " dem damaligen Fylke [[-Trøndelag]] an. Dieses ging im Zuge der [[Regionalreform in Norwegen]]" \
                    " in das zum 1. Januar 2018 neu geschaffene Fylke " + fylke_name + " über."
        else:
            text += "31. Dezember 2019 gehörte " + self.name + \
                    " dem damaligen Fylke [[]] an. Dieses ging im Zuge der [[Regionalreform in Norwegen]] in das " \
                    "zum 1. Januar 2020 neu geschaffene Fylke " + fylke_name + " über."
    ...
    def write_tettsteder(self):
        return tettsted.write_tettsteder_muni(self.muni_nr, self.name)
    ...
    def write_language(self):
        ...
        if language == "bokmål":
            if fylke_nr in ("34", "30", "18", "54"):  # Innlandet, Viken, Nordland, ToF
                text = "Offizielle Schriftsprache ist wie in vielen Kommunen in " + fylke_name +\
                       " [[Bokmål]], die weiter verbreitete der beiden norwegischen Sprachformen."
            elif fylke_nr in ("38", "11"):  # VoT, Rogaland
                text = "Offizielle Schriftsprache ist wie in nur wenigen Kommunen in " + fylke_name +\
                       " [[Bokmål]], die weiter verbreitete der beiden norwegischen Sprachformen."
            elif fylke_nr in ("42", "50"):
                text = "Offizielle Schriftsprache ist wie in einigen weiteren Kommunen in " + fylke_name +\
                       " [[Bokmål]], die weiter verbreitete der beiden norwegischen Sprachformen."

        elif language == "nynorsk":
            if fylke_nr in ("34", "30"):  # Innlandet, Viken
                text = "Offizielle Schriftsprache ist wie in nur wenigen Kommunen in " + fylke_name + \
                       " [[Nynorsk]], die weniger weit verbreitete der beiden norwegischen Sprachformen."
            ...
            if fylke_nr in (
            "34", "18", "54", "38", "42", "11", "50"):  # Innlandet, Nordland, ToF, VoT, Agder, Rogaland, Trøndelag
                text = self.name + " hat wie viele andere Kommunen der Provinz " + fylke_name + \
                       " weder [[Nynorsk]] noch [[Bokmål]] als offizielle Sprachform, sondern " \
                       "ist in dieser Frage neutral."
            elif fylke_nr in ("30", "15"):  # Viken, Møre
                text = self.name + " hat wie einige weitere Kommunen der Provinz " + fylke_name +\
                       " weder [[Nynorsk]] noch [[Bokmål]] als offizielle Sprachform," \
                       " sondern ist in dieser Frage neutral."
            elif fylke_nr == "46":  # Vestland
                text = "Im Gegensatz zu den meisten anderen Kommunen der Provinz " + fylke_name + " hat " + self.name \
                       + " weder [[Nynorsk]] noch [[Bokmål]] als offizielle Sprachform, sondern ist in " \
                         "dieser Frage neutral."
            elif fylke_nr == "03":
                text = self.name + " hat weder [[Nynorsk]] noch [[Bokmål]] als offizielle Sprachform, sondern ist " \
        ...
        if kartverketAPI.is_samisk_forvaltningsomrade(self.code):
            text += " Da " + self.name + \
                    " Teil des samischen Verwaltungsgebiets ist, ist die norwegische Sprache dem " \
                    "[[Samische Sprachen|Samischen]] gleichgestellt. Die Einwohner haben dadurch unter anderem einen " \
                    "Anspruch darauf, die Kommunikation mit öffentlichen Organen in einer samischen Sprache " \
                    "laufen zu lassen." + reference.get_source_reg_sami()
    ...
    def write_commuters(self):
        return "Im Jahr 2021 arbeiteten von rund x Arbeitstätigen etwa x in " + self.name + " selbst, ." +\
               reference.get_source_ssb_commuter()
    ...
    def write_name(self):
        return self.name + " wurde im Jahr ... als ''...'' erwähnt. Der Name setzt sich aus den beiden " \
                           "Bestandteilen „...“ und „...“ zusammen, erster leitet sich von ... ab, „...“ steht " \
                           "hingegen für „...“." + reference.get_source_stadnamn(self.name)
