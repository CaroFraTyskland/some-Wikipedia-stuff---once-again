...
def __check_article(article):
    url = "https://de.wikipedia.org/w/api.php?action=query&prop=categories&titles=" + article + "&cllimit=max&format" \
                                                                                                "=json "
...
def get_cats_from_article(article):
    __check_article(article)

    return categories.sort()


# returns a list of categories from several articles
def get_cats_from_articles(articles):
    articleString = "|".join(articles)

    url = "https://de.wikipedia.org/w/api.php?action=query&prop=categories&titles=" + articleString + "&cllimit=max&format=json"
